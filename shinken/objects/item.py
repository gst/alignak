...
"""
import time
import pickle  # for hashing compute
import itertools
...
Like temporary attributes such as "imported_from", etc.. """
    ...
    def compute_hash(self):
        ...
        del self.id
        m = md5()
        tmp = pickle.dumps(self, pickle.HIGHEST_PROTOCOL)
        m.update(tmp)
        self.hash = m.digest()
...
class Items(object):
    ...
    # Nagios3 ones
    def old_properties_names_to_new(self):
        for i in itertools.chain(iter(self.items.values()),
                                 iter(self.templates.values())):
